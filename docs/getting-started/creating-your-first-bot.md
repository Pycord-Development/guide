...
Excited to create your first bot? Once you [install Pycord](../installation.md), you can start right
away!
...
1. Give your bot a name, and click <button class="blurplebutton">Create</button>.
...
1. Click on the <button class="greybutton">Bot</button> tab on the left side of the screen.
1. Click on <button class="blurplebutton">Add bot</button>.
1. You can give it a name, change the Avatar, etc.
...
1. Click on the <button class="blurplebutton">Copy</button> button in the "Token" section.
...
1. Define the token in the `.env` file (replace the example value with your token).
    ```env title=".env"
...
1. Load the token from the `.env` file.
    ...
    ```
1. Pass your token as parameter when running the bot.
    ```python
    client.run(token)
    ```
...
```
# link with creating-first-bot-hello.json
```
...
!!! note
    ...
    ??? example "A Basic Bot without securing the token"
...
if the message starts with a certain character. You can read [this page](../extensions/commands/prefixed-commands.md)
to learn more about prefixed commands.

### How do I setup modules/cogs?

[Cogs](../popular-topics/cogs.md) are a great way to organize your commands by putting them into
...
To learn more, read about Message Commands in our [interactions directory](../interactions/index.md).

!!! info "Related Topics"

    - [Prefixed Commands](../extensions/commands/prefixed-commands.md)

...
ext.commands comes with it's own built-in help commands. In this guide, we will take a look at them as well as learn how to create your own. Let's dive in!
...
- We get the name of the cog using `getattr(cog, "qualified_name", "No Category")`. This calls the attribute `qualified_name` of the cog, and if it has no name, it returns "No Category".

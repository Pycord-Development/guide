...
Then we create the `set_time_between_updates` sub-command which takes in the time in minutes and changes the time between updates of the leaderboard, the `stop` sub-command which stops the task and `restart` which restarts the task.

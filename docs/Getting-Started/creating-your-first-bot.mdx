...
pattern, snowflakes in computers are unique things - no two bots have the same token - so a token is considered a snowflake. A Discord user ID
is also a snowflake.
...
We create a new instance of [`discord.Bot`](https://docs.pycord.dev/en/master/api.html#discord.Bot). In the `debug_guilds` argument, we pass a list of guild IDs where we want to test the bot. Remember to change the
...
Now, you are done with creating your first bot! This is the basic structure of a Pycord bot. What else can you do? The sky is
...
You can read [this amazing page that doesn't exist](../Extensions/Commands/prefixed-commands) and idk how it is gonna be linked

...
bot = pycord.Bot(intents=pycord.Intents(guild_members=True, guild_bans=True))
```
...
Enabling unneeded intents could also lead to excessive memory usage which would likely lead to crashes and midnight headaches.

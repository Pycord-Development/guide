...
class Greetings(
    ...
    math = discord.SlashCommandGroup(
        "math", "Spooky math stuff"
    )  # create a Slash Command Group called "math"
    advanced_math = math.create_subgroup("advanced", "super hard math commands!")
    ...
    async def midpoint(self, ctx, x1: float, y1: float, x2: float, y2: float):
        ...
        mid_y = (y1 + y2) / 2
        await ctx.respond(
            f"The midpoint between those coordinates is ({mid_x}, {mid_y})."
        )
...
```python
cogs_list = ["greetings", "moderation", "fun", "owner"]
...
create a help command [here](../extensions/commands/help-command.md).

!!! info "Related Topics"

    - [Creating a Help Command](../extensions/commands/help-command.md)
    ...
    - [Application Commands](../interactions/index.md#application-commands)

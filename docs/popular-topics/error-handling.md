...
async def feedback(ctx: discord.ApplicationContext, name: str):
    try:
        await ctx.respond(
            f"Hey, {name}! Send your feedback within the next 30 seconds please!"
        )
...
```
# link with feedback-response.json
# link with feedback-user-message.json
# link with feedback-thanks.json
```
...
```
# link with feedback-timeout-response.json
# link with feedback-timeout-error.json
```
...
## Per-Command Handling { #percommand }
...
```
# link with secret-non-owner-response.json
```
...
```
# link with avatar-dm-user-msg.json
# link with avatar-dm-response.json
```
...
```
# link with avatar-guild-user-msg.json
# link with avatar-guild-error.json
```
...
```
# link with ping-cooldown-first.json
# link with ping-cooldown-error.json
```
...
!!! info "Related Topics"
    ...
    - [Slash Commands](../interactions/application-commands/slash-commands.md)

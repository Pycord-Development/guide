...
async def button(ctx):
    await ctx.send("Press thebutton!", view=MyView())
```
...
class MyView(discord.ui.View):
  @discord.ui.select(options = [...])
    async def select_callback(self, select, interaction):        select.disabled = True # set the status of the select as disabled

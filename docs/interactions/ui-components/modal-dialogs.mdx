...
Modal Dialogs, also known as "modals", provide a form-like structure for bots to recieve input from users. No extra permissions are needed for bots to send modal dialogs, so it becomes a convinient workaround for bots to recieve longer input without needing the Message Content [intent](../../popular-topics/intents). 
...
The `ctx` parameter we define in Application Commands recieves an [`ApplicationContext`](https://docs.pycord.dev/en/master/api.html#discord.ApplicationContext) object. Using its `send_modal()` coroutine, you can send a Modal dialog to the user invoking the Application Command.

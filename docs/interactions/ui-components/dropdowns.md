...
Select Menus aren't the only message component in Discord. There's also [Buttons](buttons.md) and [Modal Dialogs](modal-dialogs.md). Select Menus make it easy for users to pick one or multiple options from a list provided by a bot.
    ...
    class MyView(discord.ui.View):
        ...
        async def select_callback(self, select, interaction):
            ...
            await interaction.response.edit_message(
                view=self
            )  # edit the message to show the changes
    ```
...
async def select_menu(ctx):
    await ctx.send(
        f"View persistence status: {MyView.is_persistent(MyView())}", view=MyView()
    )
```

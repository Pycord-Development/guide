...
Buttons weren't the only update to the interactions system in Discord. Discord also added [Select Menus](dropdowns.md) and [Modal Dialogs](modal-dialogs.md), both of which work very similarly to buttons.

These UI elements reside in a "view". To learn more about views, please refer to the
[interactions page](../index.md).
...
```
# link with buttons-button-response.json
```
...
class MyView(discord.ui.View):
    @discord.ui.button(
        label="A button", style=discord.ButtonStyle.primary, disabled=True
    )  # pass `disabled=True` to make the button pre-disabled
    async def button_callback(self, button, interaction): ...
...
=== "Disabling a single component"
    ...
    class MyView(discord.ui.View):
        ...
        async def button_callback(self, button, interaction):
            ...
            button.label = (
                "No more pressing!"  # change the button's label to something else
            )
    ...
    class MyView(discord.ui.View):
        async def on_timeout(self):
            ...
            await self.message.edit(
                content="You took too long! Disabled all the components.", view=self
            )
...
async def button(ctx):
    await ctx.send(
        f"Press the button! View persistence status: {MyView.is_persistent(MyView())}",
        view=MyView(),
    )
```
...
#### What is OOP? What is subclassing? { #oop }
...
!!! info "Related Topics"
    ...
    - [Slash Commands](../application-commands/slash-commands.md)

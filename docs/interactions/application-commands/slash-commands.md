...
```
# link with slash-commands-ping.json
```
...
!!! info "Cogs"
    ...
    If you are looking to add Slash Command Groups to cogs, please look at our [Cogs page](../../popular-topics/cogs.md)!
...
=== "Using Type Annotations"
    ...
    ```
    # link with slash-commands-add.json
    # link with slash-commands-join.json
    ```
...
=== "Using option decorator"
    ...
    @bot.command()
    @discord.option(
        "first", type=discord.SlashCommandOptionType.string
    )  # type = str also works
    @discord.option(
        "second", type=discord.SlashCommandOptionType.string
    )  # type = str also works
    async def join(
        ctx,
        first: str,
...
```
# link with slash-commands-animal.json
```

...
These UI elements reside in a "view". A view can have 5 rows, each row having 5 slots. Each button takes up one slot, and a dropdown takes up all 5 slots of a row. This means that a maximum of 25 buttons can be added to a view. Each message can only have one view.
...
As you can see in the last line, we set a timeout of 30 seconds to the view. If you are reusing the view and want the timeout to stay the same, you can also define the timeout in the view itself.
